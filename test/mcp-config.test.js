...
describe('MCPConfigManager', () => {
  ...
  describe('constructor', () => {
    test('creates instance with test mode detection', () => {
      expect(manager).toBeDefined();
      expect(manager.testMode).toBeDefined();
    });
  });
  ...
  describe('buildMCPConfig', () => {
    ...
    test('includes CloudWatch server when enabled', () => {
      ...
      expect(config.mcpServers['awslabs.cloudwatch-mcp']).toBeDefined();
      expect(config.mcpServers['awslabs.cloudwatch-mcp'].command).toBe('uvx');
    });
    ...
    test('CloudWatch MCP only included when explicitly enabled', () => {
      process.env.AWS_ACCESS_KEY_ID = 'AKIATEST123';
      process.env.AWS_SECRET_ACCESS_KEY = 'secretkey';
      ...
      expect(config.mcpServers['awslabs.cloudwatch-mcp']).toBeUndefined();
    });
  ...
  describe('hasCloudWatchAccess', () => {
    test('returns true when enabled and credentials present', () => {
      process.env.ENABLE_CLOUDWATCH_MCP = 'true';
      process.env.AWS_ACCESS_KEY_ID = 'AKIATEST123';
      process.env.AWS_SECRET_ACCESS_KEY = 'secretkey';
    ...
    test('returns false when enabled but no credentials', () => {
      process.env.ENABLE_CLOUDWATCH_MCP = 'true';
      delete process.env.AWS_ACCESS_KEY_ID;
      delete process.env.AWS_SECRET_ACCESS_KEY;
  ...
  describe('server configuration getters', () => {
    ...
    test('getCloudWatchServerConfig returns valid config', () => {
      ...
      expect(config.command).toBe('uvx');
      expect(config.args).toContain('awslabs.cloudwatch-mcp-server@latest');
    });
  });
  ...
  describe('tool lists', () => {
    ...
    test('getCloudWatchToolsList returns array', () => {
      const tools = manager.getCloudWatchToolsList();

      expect(Array.isArray(tools)).toBe(true);
      expect(tools.length).toBeGreaterThan(0);
      expect(tools[0]).toContain('mcp__awslabs_cloudwatch-mcp-server__');
    });
